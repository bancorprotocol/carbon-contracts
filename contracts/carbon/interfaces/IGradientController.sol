...
import { GradientStrategy, TradeAction, GradientOrder } from "../GradientStrategies.sol";

/**
 * @dev Carbon Controller interface
 */
interface IGradientController is IUpgradeable {
    /**
     ...
     */
    function createStrategy(Token token0, Token token1, GradientOrder calldata order) external payable returns (uint256);

    /**
     * @dev updates an existing strategy
     *
     * notes:
     * - currentOrder should reflect the order values at the time of sending the tx
