...
import { GradientStrategies, GradientStrategy, TradeAction, GradientOrder, TradeTokens } from "./GradientStrategies.sol";
import { Upgradeable } from "../utility/Upgradeable.sol";
import { IVoucher } from "../voucher/interfaces/IVoucher.sol";
import { IGradientController } from "./interfaces/IGradientController.sol";
...
/**
 * @dev Gradient Controller contract
 */
contract GradientController is
    IGradientController,
    Pairs,
    GradientStrategies,
    Upgradeable,
    ...
    Utils
{
    ...
    /**
     * @inheritdoc IGradientController
     */
    ...
    function createStrategy(
        ...
        // revert if the order is invalid
        ...
        return _createStrategy(_voucher, tokens, order, strategyPair, msg.sender, msg.value);
    }
    ...
    function updateStrategy(
        ...
        // only the owner of the strategy is allowed to update it
        if (msg.sender != _voucher.ownerOf(strategyId)) {
            revert AccessDenied();
        }

        // don't allow unnecessary eth
        // @TODO: fix token ordering and check only for token 1
        ...
        // perform update
        _updateStrategy(strategyId, currentOrder, newOrder, strategyPair, msg.sender, msg.value);
    }
     ...
     * @inheritdoc IGradientController
     */
    function strategy(uint256 id) external view returns (GradientStrategy memory) {
    ...
    function strategiesByPair(
        ...
        uint256 startIndex,
        uint256 endIndex
    ) external view returns (GradientStrategy[] memory) {
        _validateInputTokens(token0, token1);
